...
async def keitaro_postback(request: Request, authorization: str | None = Header(default=None)):
    ...
    buyer_id = await db.find_user_for_postback(
        offer=data.get("offer") or data.get("offer_name") or data.get("campaign"),
        country=data.get("country") or data.get("geo"),
        source=data.get("source") or data.get("traffic_source") or data.get("affiliate")
    )
    ...
    offer = data.get("offer") or data.get("offer_name")
    country = data.get("country") or data.get("geo")
