...
async def _download_youtube_video(url: str) -> YoutubeDownloadResult:
    ...
    if probe is None:
        ...
        error_msg = str(last_probe_error) if last_probe_error else "Не удалось получить информацию о видео"
        
        # Улучшаем сообщение об ошибке для возрастных ограничений
        if last_probe_error:
            error_str = str(last_probe_error).lower()
        ...
        raise YoutubeDownloadError(error_msg) from last_probe_error
